...
from autoevolve.constants import ROOT_FILES, SUPPORTED_HARNESSES
...
from autoevolve.utils import resolve_repo_path, write_text_file
...
def run_init(args: list[str]) -> None:
    ...
    if mode == "now":
        ...
        metric = require_filled_value(
            options.metric or ask_metric_spec(""),
            "metric",
        )
        try:
        ...
        validation = require_filled_value(
            options.validation or ask_validation(""),
            "validation",
        )
    ...
    from autoevolve.constants import HARNESS_PATHS
